...
public class PlayerHUD {
    ...
    public static void render(MatrixStack g, float partialTick, int width, int height) {
        ...
        com.wavedefense.gui.GuiCompat.drawString(g, mc.font, pointsText, width - textWidth - 10, height - 20, 0xFFFFFF);
        ...
        if (currentLoc != null
            ...
            com.wavedefense.gui.GuiCompat.drawString(g, mc.font, waveText, width - ww - 10, height - 50, 0xFFE0A020);
        }
        ...
        if (data.getVictoryCountdownSec() <= 0 && data.isTimerActive() && data.isShowTimer()) {
            ...
            com.wavedefense.gui.GuiCompat.drawString(g, mc.font, timerText, width - tw - 10, height - 35, 0xFFFFFF);
        }
     ...
     */
    private static void renderDmLeaderboard(MatrixStack g, Minecraft mc, int width,
        ...
        // Background panel
        com.wavedefense.gui.GuiCompat.fill(g, x - 4, y - 2, x + w + 4, y + 30, 0x80000000);
        com.wavedefense.gui.GuiCompat.drawString(g, mc.font, line1, x, y,      0xFFE0A020);
        com.wavedefense.gui.GuiCompat.drawString(g, mc.font, line2, x, y + 10, 0xFFCFCFCF);
        com.wavedefense.gui.GuiCompat.drawString(g, mc.font, line3, x, y + 20, 0xFF80FF80);
    }
    ...
    private static void renderTeamPanel(MatrixStack g, Minecraft mc) {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            com.wavedefense.gui.GuiCompat.fill(g, X - 1, y - 1, X + rowW + 1, y + ROW_H - 2, isMe ? 0x44004400 : 0x44000000);

            // ── Нік (обрізаний) ───────────────────────────────────
            String prefix = isMe ? "§a" : (alive ? "§f" : "§8");
            String name   = prefix + truncate(e.name(), 8);
            com.wavedefense.gui.GuiCompat.drawString(g, mc.font, name, X, y + 1, 0xFFFFFF, false);
            ...
            // Фон
            com.wavedefense.gui.GuiCompat.fill(g, bx, by, bx + BAR_W, by + BAR_H, 0xFF1A1A1A);
            ...
            if (alive && e.maxHp() > 0) {
                ...
                int col = ratio > 0.5f ? 0xFF3CC93C
                        ...
                        : 0xFFE03030;
                com.wavedefense.gui.GuiCompat.fill(g, bx, by, bx + filled, by + BAR_H, col);
            } else if (!alive) {
                // Червоний крест замість бару
                com.wavedefense.gui.GuiCompat.fill(g, bx, by, bx + BAR_W, by + BAR_H, 0x88440000);
                int cx2 = bx + BAR_W / 2;
                com.wavedefense.gui.GuiCompat.fill(g, cx2 - 1, by,     cx2 + 1, by + BAR_H, 0xFFCC2222);
                com.wavedefense.gui.GuiCompat.fill(g, bx,      by + 1, bx + BAR_W, by + 3,  0xFFCC2222);
            }

            // Рамка бару (1px)
            com.wavedefense.gui.GuiCompat.fill(g, bx,            by,          bx + BAR_W, by + 1,        0xFF555555);
            com.wavedefense.gui.GuiCompat.fill(g, bx,            by + BAR_H - 1, bx + BAR_W, by + BAR_H, 0xFF555555);
            com.wavedefense.gui.GuiCompat.fill(g, bx,            by,          bx + 1,     by + BAR_H,    0xFF555555);
            com.wavedefense.gui.GuiCompat.fill(g, bx + BAR_W - 1, by,         bx + BAR_W, by + BAR_H,   0xFF555555);
        }
    }
     ...
     */
    private static void renderCtpOverlay(MatrixStack g, Minecraft mc, int screenW) {
        ...
        for (String pointId : owners.keySet()) {
            ...
            // Background panel
            com.wavedefense.gui.GuiCompat.fill(g, x - 2, startY - 1, x + ENTRY_W + 2, startY + ROW_H + 1, 0x88000000);

            // Point label
            String label = truncate(displayName, 10);
            int lw = mc.font.width(label);
            com.wavedefense.gui.GuiCompat.drawString(g, mc.font, label, x + (ENTRY_W - lw) / 2, startY, 0xFFFFFFFF, false);
            ...
            com.wavedefense.gui.GuiCompat.drawString(g, mc.font, ownerLabel, x + (ENTRY_W - ow) / 2, startY + 8, ownerColor, false);

            // Capture progress bar (only shown when being contested, i.e. prog > 0 and < 100)
            int barY = startY + ROW_H - BAR_H;
            com.wavedefense.gui.GuiCompat.fill(g, x, barY, x + ENTRY_W, barY + BAR_H, 0xFF222222);
            if (prog > 0) {
                int filled = (int)(ENTRY_W * (prog / 100.0f));
                com.wavedefense.gui.GuiCompat.fill(g, x, barY, x + filled, barY + BAR_H, 0xFFFFD700);
            }
        ...
        if (!score.isEmpty()) {
            ...
            com.wavedefense.gui.GuiCompat.drawString(g, mc.font, scoreText, (screenW - sw) / 2, startY + ROW_H + 3, 0xFFFFFFFF, false);
        }
    }

    /** Returns a distinct colour for a team name (consistent across calls). */
    private static int teamColor(String team) {
        if (team == null || team.trim().isEmpty()) return 0xFFAAAAAA;
